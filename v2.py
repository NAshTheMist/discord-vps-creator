...
@bot.tree.command(name="deploy", description="Creates a new Instance with custom resources")
@app_commands.describe(ram="Amount of RAM (e.g., 2g, 4g, etc.)", disk="Disk size (e.g., 10G, 20G, etc.)", cpu="Number of CPU cores")
async def deploy(interaction: discord.Interaction, ram: str, disk: str, cpu: int):
    await interaction.response.send_message(embed=discord.Embed(description="Creating Instance, This takes a few seconds.", color=0x00ff00))
    user = str(interaction.user)
    ...
    try:
        container_id = subprocess.check_output([
            ...
            f"--memory={ram}", f"--cpus={cpu}", f"--storage-opt=size={disk}", image
        ]).strip().decode('utf-8')
    except subprocess.CalledProcessError as e:
        await interaction.followup.send(embed=discord.Embed(description=f"Error creating Docker container: {e}", color=0xff0000))
        return
    
    ssh_session_line = f"ssh user@{container_id}"
    add_to_database(user, container_id, ssh_session_line)
    await interaction.user.send(embed=discord.Embed(description=f"### Successfully created Instance\nSSH Session Command: ```{ssh_session_line}```\nOS: Ubuntu 22.04\nRAM: {ram}, CPU: {cpu} cores, Disk: {disk}", color=0x00ff00))
    await interaction.followup.send(embed=discord.Embed(description="Instance created successfully. Check your DMs for details.", color=0x00ff00))

@bot.tree.command(name="deploy", description="Creates a new instance with custom resources")
@app_commands.describe(
    os_type="Choose OS: 'ubuntu' or 'debian'",
    ram="Amount of RAM (e.g., 2g, 4g, etc.)",
    disk="Disk size (e.g., 10G, 20G, etc.)",
    cpu="Number of CPU cores"
)
async def deploy(interaction: discord.Interaction, os_type: str, ram: str, disk: str, cpu: int):
    await interaction.response.send_message(embed=discord.Embed(description="Creating Instance, This takes a few seconds.", color=0x00ff00))
    ...
    if os_type.lower() == "ubuntu":
        ...
        await interaction.followup.send(embed=discord.Embed(description="Invalid OS type! Choose 'ubuntu' or 'debian'.", color=0xff0000))
        return

    try:
        # Run Docker with specified RAM, CPU, and Disk
        container_id = subprocess.check_output([
            "docker", "run", "-itd", "--privileged", "--cap-add=ALL",
            f"--memory={ram}", f"--cpus={cpu}", f"--storage-opt=size={disk}", image
        ]).strip().decode('utf-8')
    ...
    if ssh_session_line:
        ...
        await interaction.followup.send(embed=discord.Embed(description="Instance creation failed. Try again later.", color=0xff0000))
        subprocess.run(["docker", "kill", container_id])
        subprocess.run(["docker", "rm", container_id])
